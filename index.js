...
// Validation
function validateInput(e, max, min) {
    let MAX_VALUE = max;
    let MIN_VALUE = min;
    let value = e.value;
    let pattern1 = /^\d{0,2}$/;
    let pattern2 = /^\d*$/;
    let pattern3 = /^0?\.\d{0,2}$/;
    let pattern4 = /^0?/;
    let parent = e.parentElement;
    let lastElement = parent.lastElementChild;

    if (!min) {
        if (pattern2.test(value) && parseInt(value) > MAX_VALUE) {
            let errorMaxValue = createError("Не больше 40");
            if (lastElement.classList.contains('errorMessage')) {
                return;
            }
            parent.appendChild(errorMaxValue);
            return;
        }
        if (!pattern1.test(value)) {

            let errorNotNumber = createError("Введите цифру");
            if (lastElement.classList.contains('errorMessage')) {
                return;
            }
            parent.appendChild(errorNotNumber);
            return;
        } else if (lastElement.classList.contains('errorMessage')) {
    ...
    if(min && max) {
        ...
        if (!pattern4.test(value)) {
            ...
            let errorNotNumber = createError("0.01 ... 0.99");
            if (lastElement.classList.contains('errorMessage')) {
                return;
            }
            parent.appendChild(errorNotNumber);
